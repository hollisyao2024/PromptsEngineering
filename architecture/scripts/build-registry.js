...
function buildRegistry(source=path.resolve(__dirname,'../..')) {
  ...
  const build=item=>{
    const visited=new Set(),files=new Map(),dependencies=new Set(['clsx','tailwind-merge','class-variance-authority','react','react-dom','lucide-react','radix-ui'].map(name=>`${name}@${versions[name]}`));
    function include(name){if(visited.has(name))return;visited.add(name);const current=index.get(name);if(!current)throw new Error(`Registry dependency missing: ${name}`);
      ...
      for(const dep of current.dependencies||[])dependencies.add(dep);
    ...
    include(item.name);
    files.set('utils.ts',{path:'utils.ts',type:'registry:lib',target:'@lib/utils.ts',content:'import { clsx, type ClassValue } from "clsx";\nimport { twMerge } from "tailwind-merge";\nexport function cn(...inputs: ClassValue[]) { return twMerge(clsx(inputs)); }\n'});
