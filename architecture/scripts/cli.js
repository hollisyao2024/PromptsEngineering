...
const { catalog, validateConfig, detectProject, createArchitecturePlan, CONFIG, walk } = require('./project');
...
function args(argv) {
  const result={action:argv[0]||'help'}; const values=new Set(['target','source','config','out','plan','scope','run-root']);
...
function main(argv=process.argv.slice(2)) {
  ...
  if(cli.action==='help') {console.log('Usage: pnpm agent -- architecture <catalog|detect|validate|plan|init|update|adopt|apply|resume|check|install-deps> [--target path] [--config file] [--scope owner] [--out file] [--plan file] [--dry-run] [--no-install]\nStandalone: node architecture/scripts/cli.js <action>');return;}
  ...
  const config=validateConfig(parseJson(fs.readFileSync(configFile,'utf8'),CONFIG),{target,source});
