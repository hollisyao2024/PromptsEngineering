...
目录中的可用项不等于项目已安装项。3.4 轻量入口只保留选型 metadata；未选实现、示例和测试留在源或容器缓存。项目选择 componentSets/modules/fileStorage 后才生成对应代码和依赖闭包，普通 template sync 只维护已采用能力。需要卸载已采用组件时，仍由项目显式处理引用与数据迁移。
