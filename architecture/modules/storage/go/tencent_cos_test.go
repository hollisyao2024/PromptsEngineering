...
func TestNewTencentCOSSignature(t *testing.T) {
	calls := 0
	...
	if !strings.Contains(u.Query().Get("q-header-list"), "content-length") || signed.Headers["Content-Length"] != "" {
...
func TestLiveNewTencentCOS(t *testing.T) { liveCloud(t, "tencent-cos", NewTencentCOS) }
