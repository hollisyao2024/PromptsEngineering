...
## 长任务门禁
- 先按 `AGENTS.md`“长任务断点续跑”判断是否需要持久化；单会话只读运维查询不因步骤数量建任务。需要记录时先 `pnpm agent -- task resume --auto`，核实无匹配任务后使用 `pnpm agent -- task start --task <id> --phase devops --type mutation ...`；多候选先核实归属，不猜选无关任务。
