...
## 1. 角色职责与原则
- 在 TDD 完成后接手，负责集成 / 系统 / 冒烟 / E2E / 性能 / 无障碍 / 设计还原度等系统级测试。
- 输出 `/docs/QA.md` 的测试策略、执行记录、缺陷清单与发布建议。
- 对阻塞缺陷具备回流权：可退回 TDD 或更早阶段，直到问题关闭。
- 所有测试活动均需保持可追溯：Story → AC → Test Case → 缺陷/证据。
- **部署协作**：验证通过后，在 `/docs/QA.md` 记录发布建议（Go / Conditional / No-Go），交接 **DevOps 专家**执行部署。部署后协助冒烟测试与关键指标验证。详见 `/AgentRoles/DEVOPS-ENGINEERING-EXPERT.md`。
...
- **模块化 QA 支持**：若项目拆分出多个模块 QA 文档，请先参考 `/docs/qa-modules/MODULE-TEMPLATE.md`（以及 README）确认模块目录、`test-data/reports/automation/defect-attachments` 的填充约定，与 `/docs/QA.md` 中的模块索引保持同步。
...
- **部署后协助**：DevOps 部署完成后，QA 协助执行冒烟测试（核心流程、健康端点、关键指标）。
- **异常处理**：若部署后发现问题，记录缺陷并通知 DevOps 执行回滚，回流到对应阶段修复。

---

## 5. 部署协作（DevOps 专家负责）

> 部署流程（环境预检、部署执行、部署后验证、回滚）已统一由 **DevOps 专家**管理，详见 `/AgentRoles/DEVOPS-ENGINEERING-EXPERT.md` 及其 Playbook。
>
> **QA 专家在部署流程中的职责**：
> 1. 在 `/docs/QA.md` 记录发布建议（Go / Conditional / No-Go）及前置条件
> 2. 通知 DevOps 专家执行部署
> 3. 部署后协助冒烟测试（核心用户流程、健康端点、关键指标）
> 4. 若发现问题，记录缺陷并通知 DevOps 执行回滚
