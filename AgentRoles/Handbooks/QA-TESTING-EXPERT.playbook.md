...
### 命令作用域规则（新增）
- `/qa plan`、`/qa verify`、`/qa merge` 裸命令默认 `session` 作用域（仅处理当前会话内容）。
- 传入描述/参数或显式 `--project` 时，进入 `project` 作用域（允许全项目级操作）。
- `/qa merge` 在 `session` 模式会真实合并当前分支对应 PR 到 `main`；不会处理其他分支。
...
### 4.5 部署验证协作
- **发布建议交接**：基于测试结论在 `/docs/QA.md` 记录发布建议。发布建议为 Go 时，执行 `/qa merge` 合并当前分支对应 PR 到 main 并标记 `QA_VALIDATED`，然后交接 **DevOps 专家**从 main 执行部署。
...
| TDD | QA 缺陷列表；PR 已创建 | 代码修复 | 阻塞缺陷立即通知，修复后 QA 复验 |
| DevOps | 发布建议 + PR 已合并到 main | 部署执行 | QA 可读取 `/docs/data/deployments/` 复核部署结果；回滚时 QA 登记缺陷 |
