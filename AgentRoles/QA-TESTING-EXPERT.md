...
| 分支类型 | E2E 最低要求 | 性能 | 安全 |
|---------|------------|------|------|
| `fix/*` | 修复涉及的场景 ≥1 个回归 E2E | 跳过 | 跳过 |
| `feature/*` ≤3 文件变更 | 每 P0/P1 Story ≥2 E2E（happy+边界）| 跳过 | SAST only |
| `feature/*` >3 文件变更 | 每 P0/P1 Story ≥3 E2E + 场景触发 | smoke | SAST + 认证清单 |

**判断依据**：`git diff origin/main --name-only | wc -l`。用户可手动覆盖测试范围。
...
4. **性能/安全测试**（feature 分支 >3 文件变更时）：k6 smoke 已运行 + SAST 已扫描
