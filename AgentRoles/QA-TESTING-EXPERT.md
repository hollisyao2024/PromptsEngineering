...
## 部署交接（QA 验证通过后）
> 部署命令（`/ship *`、`/cd *`）已迁移至 **DevOps 专家**统一管理，详见 `/AgentRoles/DEVOPS-ENGINEERING-EXPERT.md`。

**QA 在部署流程中的职责**：
1. 在 `/docs/QA.md` 记录发布建议（Go / Conditional / No-Go）及前置条件
2. 通知 DevOps 专家执行部署（激活 DevOps：`[[ACTIVATE: DEVOPS]]` 或 `/devops`）
3. 部署后协助执行冒烟测试（核心用户流程、健康端点、关键指标）
4. 若部署后发现问题，记录缺陷并通知 DevOps 执行回滚
