...
_execute_remote_migration() {
    ...
    ssh ${SSH_OPTS} "$SERVER_USER@$SERVER_HOST" << ENDSSH
        ...
        STANDALONE_DIR="$REMOTE_BUILD_DIR/standalone/frontend"
        ...
        # 加载环境变量
        echo "[INFO] 加载环境变量..."
        ENV_FILE="$REMOTE_BUILD_DIR/standalone/frontend/.env"
        if [ -f "\$ENV_FILE" ]; then
            set -a
            source "\$ENV_FILE"
            set +a
        ...
        if echo "\$MIGRATION_STATUS" | grep -q "Database schema is up to date"; then
            ...
            if [[ -n "\$DRIFT_OUTPUT" ]]; then
                ...
                echo "[ERROR]   1. 检查 schema.prisma 中的期望定义"
                echo "[ERROR]   2. 运行 pnpm prisma migrate dev --create-only --name fix_drift"
                echo "[ERROR]   3. 手动编辑迁移 SQL 确保幂等性"
                echo "[ERROR]   4. 运行 pnpm prisma migrate dev 应用迁移"
                echo "[ERROR]   5. 重新部署"
                exit 1
            fi
            ...
            echo "[ERROR] 检测到迁移文件缺失！"
            echo "[ERROR] 数据库中记录了本地不存在的迁移文件"
            echo "[ERROR] 这通常意味着迁移文件未正确同步到部署目录"
            echo "[ERROR] 请检查："
            echo "[ERROR]   1. prisma/migrations 目录是否完整同步到服务器"
            echo "[ERROR]   2. 是否有迁移文件被意外删除"
            echo "[ERROR]   3. 如确认缺失迁移已被其他方式应用，请手动执行："
            echo "[ERROR]      prisma migrate resolve --applied <migration_name>"
            exit 1
        else
