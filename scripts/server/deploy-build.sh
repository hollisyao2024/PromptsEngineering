...
_check_incremental_build() {
    ...
    _verify_build_artifacts() {
        local dir="$1"
        [[ -d "$dir/standalone" ]] && \
        [[ -d "$dir/standalone/frontend" ]] && \
        [[ -d "$dir/standalone/frontend/$dir/server" ]] && \
    ...
    if [[ -d "$BUILD_DIR" ]] && _verify_build_artifacts "$BUILD_DIR"; then
        ...
        if [[ -n "$last_env_hash" ]] && [[ "$last_env_hash" != "$CURRENT_ENV_HASH" ]]; then
            ...
            if git diff --quiet -- src/ public/ package.json next.config.js prisma/schema.prisma 2>/dev/null; then
            ...
            if git diff --quiet "$last_build_hash" HEAD -- src/ public/ package.json next.config.js prisma/schema.prisma 2>/dev/null; then
...
_generate_prisma_client() {
    ...
    if [[ -f "$FRONTEND_DIR/prisma/schema.prisma" ]]; then
        ...
        if [[ "$(uname)" == "Darwin" ]]; then
            schema_file_hash=$(/sbin/md5 -q "$FRONTEND_DIR/prisma/schema.prisma")
            if [[ -d "$FRONTEND_DIR/prisma/migrations" ]]; then
                migrations_hash=$(find "$FRONTEND_DIR/prisma/migrations" -type f -exec /sbin/md5 -q {} \; | sort | /sbin/md5 -q)
            else
                migrations_hash="no-migrations"
            fi
        elif command -v md5sum &> /dev/null; then
            schema_file_hash=$(md5sum "$FRONTEND_DIR/prisma/schema.prisma" | cut -d' ' -f1)
            if [[ -d "$FRONTEND_DIR/prisma/migrations" ]]; then
                migrations_hash=$(find "$FRONTEND_DIR/prisma/migrations" -type f -exec md5sum {} \; | sort | md5sum | cut -d' ' -f1)
            else
                migrations_hash="no-migrations"
            fi
        else
...
# 构建 Cron 调度器
_build_cron_scheduler() {
    local cron_bundle_file="scripts/cron/scheduler.bundle.js"
    ...
    if [[ -f "$cron_bundle_file" ]] && [[ -f "$cron_hash_file" ]]; then
        ...
        if [[ "$(uname)" == "Darwin" ]]; then
            current_cron_hash=$(git ls-files -s scripts/cron/ src/services/credit-cache.ts src/services/order-lifecycle.ts src/lib/order-sms.ts 2>/dev/null | /sbin/md5 -q)
        elif command -v md5sum &> /dev/null; then
            current_cron_hash=$(git ls-files -s scripts/cron/ src/services/credit-cache.ts src/services/order-lifecycle.ts src/lib/order-sms.ts 2>/dev/null | md5sum | cut -d' ' -f1)
        else
...
_prepare_standalone_package() {
    ...
    if [[ "$NEED_BUILD" == "true" ]]; then
        ...
        mkdir -p "$BUILD_DIR/standalone/frontend/$BUILD_DIR"

        # 复制 server 目录
        if [[ -d "$BUILD_DIR/server" ]]; then
            cp -r "$BUILD_DIR/server" "$BUILD_DIR/standalone/frontend/$BUILD_DIR/"
        fi

        # 复制 static 目录
        if [[ -d "$BUILD_DIR/static" ]]; then
            cp -r "$BUILD_DIR/static" "$BUILD_DIR/standalone/frontend/$BUILD_DIR/"
        fi

        # 复制 public 目录
        if [[ -d "public" ]]; then
            cp -r public "$BUILD_DIR/standalone/frontend/"
        fi
    else
