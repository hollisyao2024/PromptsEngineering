...
# 用法: ./scripts/tdd-tools/create-migration.sh <description> [--dir 路径] [--dialect postgres|mysql|oracle|sqlite|generic]
# 示例: ./scripts/tdd-tools/create-migration.sh add_user_roles --dir db/migrations --dialect postgres
# ============================================================
...
usage() {
  cat <<'EOF'
用法:
  ./scripts/tdd-tools/create-migration.sh <description> [--dir 路径] [--dialect postgres|mysql|oracle|sqlite|generic]

参数:
  <description>           迁移描述（小写字母/数字/下划线）
  --dir <路径>            输出目录（默认: db/migrations）
  --dialect <方言>        数据库方言标签，用于模板注释（默认: generic）
  -h, --help              展示帮助

示例:
  ./scripts/tdd-tools/create-migration.sh add_user_roles
  ./scripts/tdd-tools/create-migration.sh add_billing_tables --dir infra/migrations --dialect mysql
  ./scripts/tdd-tools/create-migration.sh shard_user_data --dialect oracle
EOF
}
