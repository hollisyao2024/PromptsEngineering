...
- `180000` token 是自动压缩硬保险；软水位为 `80000` 正常推进、`80000-120000` 只并行独立只读检查、`120000-150000` 收口当前阶段并准备胶囊、`150000` 以上由新上下文续跑。优先自然阶段边界，不机械切断关键验证。
- 新对话始终可创建，轻量问答和状态查询不限制，长命令继续运行；`1-2 条重型任务`、单线程 `8-10 次模型请求/分钟`、活跃合计约 `2.5M TPM` 是目标水位。`3M` 告警、`4M` 对后台/heartbeat/自动恢复执行暂停、`4.5M` 只让新增重型模型请求排队；只节流模型请求，不中断 shell、测试、构建或部署。
- 新执行上下文已知任务 ID 时先执行 `pnpm agent -- task resume --task <id>`，再执行 `pnpm agent -- task context --task <id>`；仅在尚未识别任务时使用 `resume --auto`，多候选不得猜选。当前任务内正常阶段切换只刷新 context，不对仍在执行的副作用步骤调用恢复入口。只读取胶囊、当前代码和必要的章节，不全文加载 Handbook 或大型模块文档。
...
- 已获端到端授权的 PRD → ARCH → TASK → TDD → QA 流程必须自动连续推进；每次阶段转换先生成胶囊、读取下一阶段专家和必要材料，然后在当前任务中继续执行。阶段完成、角色切换或生成胶囊不是结束任务的理由，不重复要求用户确认已授权范围。
- 按上下文预算确需新执行器时，通过运行环境已获授权的交接能力传递精确任务 ID、WORKTREE 和胶囊，并收到接管确认后才释放当前执行权；只有写入新阶段或输出提示不算交接成功。不得凭此规则擅自创建用户可见的新任务、改变权限或伪称上下文已重置。缺少自动交接能力但预算允许时在当前任务中精简读取后继续；达到硬限制或遇到真实阻塞时保留状态、证据和唯一恢复入口，明确未完成。
- 连续 10 次请求中，稳定阶段的缓存命中率目标为 `70%`。可观测数据不达标时仅在授权允许下调整模型或通道；无法调整时减少重复读取并准备胶囊。没有指标不猜测命中率，不按请求次数强制结束任务；需要换上下文时遵循接管确认协议和预算限制。
- 节流不得跳过测试、关键读取、证据或验收，不降低准确性和覆盖范围；遇到 `429` 先退避、checkpoint 并保留证据，禁止重试风暴。上下文达到预算时不得偷偷截断为“完成”，必须保留日志路径、哈希、未验证状态和下一动作。
