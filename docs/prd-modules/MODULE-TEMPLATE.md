...
该表格仅作为模板，实际模块清单信息由 PRD 专家根据以上表格生成到`module-list.md`。**每次模块拆分、状态变更或交付节奏调整，都要同步更新 `module-list.md` 的“状态”“更新时间”“依赖说明”字段，并在主 PRD 的模块索引段落镜像该行内容。**为了支持 TASK/TDD/QA 的追溯，建议同时在 module list 中标注当前阶段（如“计划中”、“已确认”、“变更中”）与最新的 “交付摘要/阻塞”，保证各专家对模块节奏的统一认知。

### 2.1 模块清单的生成与同步

- **初始生成**：在开始模块化拆分前，PRD 专家可以从 `/docs/prd-modules/MODULE-TEMPLATE.md` 复制此表格并补全每个模块的名称、优先级、负责团队与指向的 `{domain}/PRD.md` 链接，生成 `docs/prd-modules/module-list.md`。此文件即是模块化项目的“目录+状态盘”，建议在 PRD 确认后立刻提交。
