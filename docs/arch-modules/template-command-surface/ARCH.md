...
## 15. 合并与工作区清理分离（US-CMDSURF-011）

qa-merge 的预检仅要求实际写入主干的目录干净；开发目录的索引与文件不是合并输入。GH merge 继续使用 match-head-commit，本地 fallback 使用回执中的 expectedHeadSha；远端刷新、base/head 复验和普通 push 不变。含本地内容的单一目录不能承担主干写入，须使用独立主干 worktree。
