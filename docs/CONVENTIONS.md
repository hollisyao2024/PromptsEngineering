...
`180000` token 是自动压缩硬保险；执行按软水位分级：`80000` 以下正常推进，`80000-120000` 只并行相互独立的只读检查，`120000-150000` 停止新的大范围调查、完成最小闭环并准备交接，`150000` 以上由新上下文从胶囊继续。优先自然阶段边界，不因单一计数机械中断仍在执行关键验证的任务。

机器级并发和速率同样是软水位，不是新会话门禁：`1-2 条重型任务`、单线程 `8-10 次模型请求/分钟`、活跃会话合计约 `2.5M TPM` 作为预留上游余量的目标；`3M` 告警、`4M` 暂停后台、heartbeat 和自动恢复，`4.5M` 只让新增重型模型请求排队。新对话始终可创建，轻量问答和状态查询不受限；只节流新增模型请求，长命令和 shell、测试、构建、部署继续运行。
