...
按测试先行顺序交付模板客户端/服务端通用命令面：先冻结负向行为，再实现配置与路由，完成相关回归后合并模板源，最后传播到 XiaoLan 验证所有权和收敛。
...
| M2-TEMPLATE | 模板实现与 QA | 2026-08-23 | 配置、路由、执行器、文档、测试 | 定向与相关全量回归通过 | 📝 待开始 |
| M3-PROPAGATE | XiaoLan 传播 | 2026-08-23 | 模板 apply 与项目配置适配 | dry-run/apply/convergence、项目规则不被覆盖 | 📝 待开始 |
...
- aliases 扩张风险：不修改模板 package scripts 命令面。
