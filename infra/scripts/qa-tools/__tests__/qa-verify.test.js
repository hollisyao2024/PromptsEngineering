...
test('validateQaFile accepts module IDs that contain digits', (t) => {
  ...
  fs.writeFileSync(
    ...
    ['# PRD', '', '## Stories', '- US-E2E-001: nightly coverage'].join('\n'),
    'utf8'
  );
  fs.writeFileSync(
    path.join(qaDir, 'QA.md'),
    ['# QA', '', '## Coverage', '- US-E2E-001 -> TC-E2E-001'].join('\n'),
    'utf8'
  );
  ...
  assert.equal(result.stats.storyCount, 1);
  assert.equal(result.stats.testCaseCount, 1);
