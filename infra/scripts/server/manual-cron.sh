...
#   ./infra/scripts/server/manual-cron.sh all           # 执行所有任务
#   ./infra/scripts/server/manual-cron.sh credit        # 仅数据一致性检查
#   ./infra/scripts/server/manual-cron.sh cleanup       # 仅账号清理
#   ./infra/scripts/server/manual-cron.sh warmup        # 仅缓存预热
...
main() {
    ...
    case "$1" in
        ...
        *)
            ...
            echo ""
            echo "使用方式："
            echo "  ./infra/scripts/server/manual-cron.sh all           # 执行所有任务"
            echo "  ./infra/scripts/server/manual-cron.sh credit        # 数据一致性检查"
            echo "  ./infra/scripts/server/manual-cron.sh cleanup       # 账号清理"
            echo "  ./infra/scripts/server/manual-cron.sh warmup        # 缓存预热"
            echo "  ./infra/scripts/server/manual-cron.sh health        # 健康检查"
            echo ""
            echo "环境变量："
            ...
            echo "  ./infra/scripts/server/manual-cron.sh all"
            echo ""
            echo "建议频率："
