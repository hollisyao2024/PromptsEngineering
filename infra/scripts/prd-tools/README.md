...
# 赋予脚本执行权限（Unix/Mac）
chmod +x infra/scripts/prd-tools/*.js
```
...
**检查项**：
- ✅ 主 PRD 必需章节完整性（检测 1~9 节并自动识别“主从总纲 + 模块”与“单文件”两种模式，若为总纲还会查找文档导航并提示缺失章节标题）
- ✅ 拆分模块 PRD 符合 `docs/prd-modules/MODULE-TEMPLATE.md` 中的结构（检查模块概述、用户故事、模块非功能需求、接口与依赖、数据模型、风险与约束、版本与变更记录、相关文档等章节）
- ✅ 追溯矩阵是否存在
- ✅ 依赖关系图是否存在
...
> ℹ️ 本工具也会依次扫描 `docs/prd-modules/*/PRD.md`，确保每个模块包含 `docs/prd-modules/MODULE-TEMPLATE.md` 中的标准章节（包含版本与变更、相关文档等），并会在控制台列出缺失的章节标题以便定位问题。
...
> 📌 `check-completeness.js` 在 v1.0 内部升级为主从/单文件模式识别，除了 1~9 节外还会检查文档导航和模块级的版本/相关文档节，缺少章节时直接在输出中列出节标题，便于对位修复。
...
{
  "scripts": {
    "your:command": "node infra/scripts/prd-tools/your-script.js"
  }
}
```
